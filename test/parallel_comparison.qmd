...
runtimes_loop <- numeric(5)
shap_loop_list <- vector("list", 5)

for (i in 1:5) {
  cat("Running iteration:", i, "\n")
  runtime_loop <- system.time({
    mtry_factor <- 1
    ...
    select_params <- select_control(number_selected = 10, drop_fraction = drop_fraction,
                                    mtry_factor = mtry_factor)
    y_num <- as.numeric(y)
    sff <- shapff(X, y_num, module_membership = moduleLabels,
  ...
  runtimes_loop[i] <- runtime_loop["elapsed"]
  
  shap_loop_df <- sff$final_SHAP
  shap_loop_df$iteration <- i
  shap_loop_list[[i]] <- shap_loop_df
}
```
```{r}
shap_loop_all <- bind_rows(shap_loop_list)
avg_loop_shap <- shap_loop_all %>%
  group_by(feature_name) %>%
  summarise(mean_importance = mean(variable_importance),
            mean_module = mean(module_membership),
            n_times_selected = n()) %>%
...
# shapleyforest (via ranger & parallel)
n_cores <- availableCores()
runtimes_fs <- numeric(5)
shap_fs_list <- vector("list", 5)

for (i in 1:5) {
  cat("Running iteration:", i, "\n")
  runtime_fs <- system.time({
    mtry_factor <- 1
    screen_params <- screen_control(drop_fraction = drop_fraction, keep_fraction = keep_fraction,
                                    mtry_factor = mtry_factor)
    select_params <- select_control(number_selected = 10, drop_fraction = drop_fraction,
  ...
  runtimes_fs[i] <- runtime_fs["elapsed"]
  
  shap_fs_df <- sff$final_SHAP
  shap_fs_df$iteration <- i
  shap_fs_list[[i]] <- shap_fs_df
}
```

```{r}
shap_fs_all <- bind_rows(shap_fs_list)
avg_fs_shap <- shap_fs_all %>%
  group_by(feature_name) %>%
  summarise(mean_importance = mean(variable_importance),
            mean_module = mean(module_membership),
            n_times_selected = n()) %>%
...
## Simulate Parallel (original)

```{r, warning = FALSE}
plan(sequential)
registerDoSEQ()  
library(shapleyforest)
rm(list = c("shapff", "shapley_forest", "shapselect_RF"))
# shapleyforest (via ranger & parallel)
n_cores <- availableCores()
runtimes <- numeric(5)
shap_og_list <- vector("list", 5)

for (i in 1:5) {
  cat("Running iteration:", i, "\n")
  runtime_og <- system.time({
    mtry_factor <- 1
    screen_params <- screen_control(drop_fraction = drop_fraction, keep_fraction = keep_fraction,
                                    mtry_factor = mtry_factor)
    select_params <- select_control(number_selected = 10, drop_fraction = drop_fraction,
  ...
  runtimes[i] <- runtime_og["elapsed"]
  
  shap_og_df <- sff$final_SHAP
  shap_og_df$iteration <- i
  shap_og_list[[i]] <- shap_og_df
}
```

```{r}
shap_og_all <- bind_rows(shap_og_list)
avg_og_shap <- shap_og_all %>%
  group_by(feature_name) %>%
  summarise(mean_importance = mean(variable_importance),
            mean_module = mean(module_membership),
            n_times_selected = n()) %>%
...
df_long <- df %>%
  pivot_longer(cols = -Run, names_to = "Method", values_to = "Runtime")

ggplot(df_long, aes(x = Run, y = Runtime, color = Method)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(title = "Runtime Comparison", x = "Run", y = "Runtime (seconds)")
```
